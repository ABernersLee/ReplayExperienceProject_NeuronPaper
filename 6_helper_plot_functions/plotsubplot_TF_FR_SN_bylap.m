function [r,p] = plotsubplot_TF_FR_SN_bylap(f,col,dat,label,yl,plotcol,datt,isNovel,isDValue,dattind,TrackType,Run,allassig,sigonly,lapind)
...
lapind2 = lapind(ind1 & ind2 & ind3 & ind4,:);
...
if ~isempty(datt)
    ...
    for iSig = 1:2
        ...
        for j=1:dat.Lapskip:min(dat.Lap,size(datt1,2))
            da = datt1(lapind2(:,j),j:j+dat.Lapskip-1,col);
            datt1(~lapind2(:,j),j:j+dat.Lapskip-1,col) = NaN;
            A(j,1)=nanmean(da(:));
        ...
        if size(A,1)==1
            [r(setdiff(1:2,iSig)),p(setdiff(1:2,iSig))]=corr(A(t)',FR(t)');
        else
            [r(setdiff(1:2,iSig)),p(setdiff(1:2,iSig))]=corr(A(t),FR(t));
        end
    end
