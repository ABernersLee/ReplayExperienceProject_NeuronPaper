function decode_spikedensity_events_OneLap_Epoch1_overlap(iident)
load(iident,'params','hp_cells','hpinterneurons','pos','vel','dirdat','spikedata','CandSeq','linposcat')


%%%%%
%%%%% Overlapping, decoding making the spikedensity sperately for each
...
Mat1=term2.*term3;
MatrixEpoch1_overlap=(Mat1./(ones(size(Mat,1),1)*sum(Mat,1)))';
clear term2 term3 Mat1
FREpochOne_overlap = FREpochOne;

save(iident,'FREpochOne_overlap','MatrixEpoch1_overlap','-append')
