function decode_spikedensity_events_remote(dday)

%%%%%
%%%%% Overlapping bins decoding of remote replay data
%%%%% Used in RunReplay_ABL_ReomteSleep.m and RunReplay_ABL_remote.m
%%%%%

VelThresh=5; Bin_Size=2.5;

fieldday = [dday '_1'];
eventday = [dday '_2'];
%use fields from fieldday to decode events and spikes from eventday
load(eventday,'hp_cells','hpinterneurons')
Cell_Number2 = max(hp_cells(~ismember(hp_cells,hpinterneurons)));
load(fieldday,'hp_cells','hpinterneurons','vel','dirdat','spikedata','cm_conv','linposcat')
%dirdat 1 is up/out, 0 is down/in
% Cell_Number = hp_cells(~ismember(hp_cells,hpinterneurons));
mmax = max([max(hp_cells(~ismember(hp_cells,hpinterneurons))) Cell_Number2]);
Cell_Number = 1:mmax;
disp(['Number of cells: ' num2str(max(Cell_Number))])
...
load(eventday,'params','hp_cells','hpinterneurons','pos','vel','dirdat','spikedata','cm_conv','linposcat')
% Cell_Number = hp_cells(~ismember(hp_cells,hpinterneurons));
...
save([dday '_3'],'replayparams','CandSeq','OutFR','InFR','Spike','Index','OutMatrix','InMatrix','Cell_Number')
