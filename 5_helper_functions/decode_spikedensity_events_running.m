function [OutMatrixRunning,InMatrixRunning,binspike] = decode_spikedensity_events_running(iident)
load(iident,'params','hp_cells','hpinterneurons','pos','vel','dirdat','spikedata','cm_conv','linposcat')

%%%%%
%%%%% Make decoding of running time in a similar way to replays to compare
%%%%% to make sure the posterior isn't bumpy like the replay bumpiness
%%%%% (Supplemental figure 6). Used in Hovers_Moving_laps.m
%%%%%
...
Check=diff([0 ; vel>VelThresh ; 0]);

DurationBound=[0.1 1e100];
SS=find(Check==1);
EE=find(Check==-1);
target=find((EE-SS)>DurationBound(1)/Time_Bin_Size & (EE-SS)<DurationBound(2)/Time_Bin_Size);
CandSeq=[Spike_Density(SS(target),2) Spike_Density(EE(target)-1,2)];

% figure; plot(vel,'k'); hold on; plot(SS(target),vel(SS(target)),'*g'); plot(EE(target),vel(EE(target)-1),'*r')
...
OutMatrixRunning = OutMatrix;
InMatrixRunning = InMatrix;
